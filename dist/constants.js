"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.GITHUB_CLIENT_SECRET = exports.GITHUB_REDIRECT_URL = exports.GITHUB_CLIENT_ID = exports.TWITTER_CLIENT_SECRET = exports.TWITTER_CLIENT_ID = exports.TWITTER_REDIRECT_URL = exports.TWITTER_API_KEY = exports.TWITTER_TOKEN = exports.TWITTER_SECRET = exports.GAUTH_TESTING_PASSWORD = exports.GAUTH_TESTING_EMAIL = exports.GAUTH_CLIENT_SECRET = exports.GAUTH_REDIRECT_URL = exports.GAUTH_CLIENT_ID = exports.__local__ = exports.__test__ = exports.__prod__ = exports.COOKIE_KEY = exports.PORT = void 0;
exports.PORT = (process.env.PORT && parseInt(process.env.PORT)) || 5000;
exports.COOKIE_KEY = 'cookey';
exports.__prod__ = process.env.NODE_ENV === 'production';
exports.__test__ = process.env.NODE_ENV === 'test';
exports.__local__ = true;
exports.GAUTH_CLIENT_ID = process.env.GAUTH_CLIENT_ID || 'TESTING_CLIENT_ID';
exports.GAUTH_REDIRECT_URL = process.env.GAUTH_REDIRECT_URL || 'http://localhost:5000';
exports.GAUTH_CLIENT_SECRET = process.env.GAUTH_CLIENT_SECRET;
exports.GAUTH_TESTING_EMAIL = process.env.GAUTH_TESTING_EMAIL;
exports.GAUTH_TESTING_PASSWORD = process.env.GAUTH_TESTING_PASSWORD;
exports.TWITTER_SECRET = process.env.TWITTER_SECRET;
exports.TWITTER_TOKEN = process.env.TWITTER_TOKEN;
exports.TWITTER_API_KEY = process.env.TWITTER_API_KEY;
exports.TWITTER_REDIRECT_URL = process.env.TWITTER_REDIRECT_URL || 'http://localhost:5000';
exports.TWITTER_CLIENT_ID = process.env.TWITTER_CLIENT_ID;
exports.TWITTER_CLIENT_SECRET = process.env.TWITTER_CLIENT_SECRET;
exports.GITHUB_CLIENT_ID = process.env.GITHUB_CLIENT_ID;
exports.GITHUB_REDIRECT_URL = process.env.GITHUB_REDIRECT_URL || 'http://localhost:5000';
exports.GITHUB_CLIENT_SECRET = process.env.GITHUB_CLIENT_SECRET;
//# sourceMappingURL=constants.js.map